...
for(i in 1:length(UserID))
{
  ...
  if(nrow(dom_symptom_before_ECV) == 0) { dom_symptom_before_ECV<- rbind(dom_symptom_before_ECV, c("NA","NA" ))}
   
   amount_symptoms_before_ECV <- nrow(dom_symptom_before_ECV)
   dom_symptom_before_ECV <- dom_symptom_before_ECV[[1,1]]
    ...
    summarise(cnt=n())%>%
     arrange(desc(cnt))
    if(nrow(dom_symptom_after_ECV) == 0) { dom_symptom_after_ECV<- rbind(dom_symptom_after_ECV, c("NA", "NA"))}

     amount_symptoms_after_ECV <- nrow(dom_symptom_after_ECV)

   dom_symptom_after_ECV <- dom_symptom_after_ECV[[1,1]]
...
write_xlsx(summary_data, "/Volumes/GoogleDrive/Shared drives/9. Clinical/9.6 Data reporting/AZM/PRE-ECV /ggplt/summary_data.xlsx")
